...
$(document).bind('ready', function(){
  if (!amplify.store('buzzwords')) {
    $.ajax({
      url: 'http://buzzwords.tladesignz.com/data.pl',
      dataType: 'jsonp',
      success: function(data) {
        amplify.store('buzzwords', data);
        buzzwords = data;
        console.log('load from net');
        render_layout(buzzwords);
      }
    });
  } else {
    console.log('load from storage');
    render_layout(amplify.store('buzzwords'));
  }

  function render_layout(buzzwords) {
    var items = [];

    $.each(buzzwords, function(key, val) {
